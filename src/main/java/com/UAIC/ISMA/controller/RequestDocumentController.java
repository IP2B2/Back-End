...
public class RequestDocumentController {
    ...
    @GetMapping("/all")
    @Operation(summary = "Get all request documents", description = "Returns a list of all documents. Optionally, filter by userId.")
    public ResponseEntity<List<RequestDocumentDTO>> getAllDocuments(
            @Parameter(description = "Optional user ID to filter requestDocument") @RequestParam(required = false) Long userId,
            @Parameter(description = "Optional access request ID to filter requestDocument") @RequestParam(required = false) Long accessRequestId
    ) {
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Get request document by ID", description = "Returns a single document by its unique ID.")
    public ResponseEntity<RequestDocumentDTO> getById(@Parameter(description = "Document ID") @PathVariable Long id) {
        return ResponseEntity.ok(requestDocumentService.findById(id));
    }

    @PostMapping
    @PreAuthorize("hasAnyAuthority('ADMIN', 'COORDONATOR')")
    @Operation(summary = "Create a new request document", description = "Creates a new document with the provided details.")
    public ResponseEntity<RequestDocumentDTO> create(
            @Parameter(description = "RequestDocument data to create") @RequestBody RequestDocumentDTO dto) {
        return ResponseEntity.ok(requestDocumentService.create(dto));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('ADMIN', 'COORDONATOR')")
    @Operation(summary = "Update a request document", description = "Updates the document with the specified ID.")
    public ResponseEntity<RequestDocumentDTO> update(
            @Parameter(description = "Document ID") @PathVariable Long id,
            @Parameter(description = "Updated document data") @RequestBody RequestDocumentDTO dto) {
        return ResponseEntity.ok(requestDocumentService.update(id, dto));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('ADMIN', 'COORDONATOR')")
    @Operation(summary = "Delete a request document", description = "Deletes the document with the specified ID.")
